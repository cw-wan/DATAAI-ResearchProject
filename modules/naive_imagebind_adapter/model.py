...
import configs.naive_imagebind_adapter_config as base_config
...
class NaiveImagebindAdapter(nn.Module):
    def __init__(self,
                 encoder_fea_dim=1024,
                 drop_out=0.5,
                 config=base_config):
        super(NaiveImagebindAdapter, self).__init__()
    ...
    def load_model(self, load_pretrain=False, load_checkpoint_epoch=None):
        if load_pretrain:
            ...
            checkpoint_path = os.path.join(self.config.Path.save,
                                           'naive_imagebind_adapter_' + str(load_checkpoint_epoch) + '.pth')
            self.load_state_dict(torch.load(checkpoint_path))

    def save_model(self, epoch=None):
        model_path = os.path.join(self.config.Path.save, 'naive_imagebind_adapter_' + str(epoch) + '.pth')
